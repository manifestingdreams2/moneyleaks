...
import { STORAGE_KEYS, DEFAULT_CREDIT, genId } from "./lib/storage";
...
const fmtDec = (n) => Number(n || 0).toLocaleString("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
function Pill({ label, color, bg }) {
  return <span style={{ fontSize: 9, color, background: bg || color + "18", padding: "2px 8px", borderRadius: 99, fontWeight: 700, letterSpacing: 0.5 }}>{String(label).toUpperCase()}</span>;
}
...
function MicroToast({ msg, color = "#00e5a0" }) {
  if (!msg) return null;
  return (
    <div style={{ background: `${color}15`, border: `1px solid ${color}30`, borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 8, marginBottom: 10, animation: "fadeIn 0.3s ease" }}>
      ...
      {!hasChange ? (
        <div style={{ textAlign: "center", padding: "16px 0" }}>
          <p style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.6 }}>
            ...
            <br />
            <span style={{ color: "#00e5a0", fontWeight: 700 }}>Add income to watch it grow.</span>
          </p>
        </div>
      ) : (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
            ...
            <div style={{ background: "#0d1f1a", border: "1px solid #00e5a025", borderRadius: 12, padding: "12px 12px" }}>
              ...
              <div style={{ fontSize: 10, color: "#64748b", marginTop: 3 }}>in 12 months</div>
              <div style={{ fontSize: 10, color: "#00e5a0", marginTop: 6 }}>▲ {fmtDec(improvedEnd - currentEnd)} difference</div>
            </div>
          </div>
          ...
          <div style={{ marginTop: 12, background: "#111218", borderRadius: 10, padding: "12px 14px", borderLeft: "3px solid #38bdf8" }}>
            <p style={{ fontSize: 12, color: "#94a3b8", lineHeight: 1.7 }}>
              <span style={{ color: "white", fontWeight: 700 }}>If you continue current habits:</span>
              <br />→ Your net worth could {currentEnd < netWorth ? "drop" : "reach"} <span style={{ color: "#f87171", fontWeight: 700 }}>{fmtDec(currentEnd)}</span> in 12 months
              <br />
              <br />
              <span style={{ color: "white", fontWeight: 700 }}>If you reduce leaks by 50%:</span>
              <br />→ Your net worth could reach <span style={{ color: "#00e5a0", fontWeight: 700 }}>{fmtDec(improvedEnd)}</span> in 12 months
            </p>
          </div>
        </>
      )}
...
// ─────────────────────────────────────────────────────────────────────────────
// DAILY SPENDING TAB (unchanged — uses its own dti_spending store)
// ─────────────────────────────────────────────────────────────────────────────
function SpendingTab() {
  const [entries, setEntries] = useLocalStorage(STORAGE_KEYS.spending, []);
  ...
  const handleDelete = (id) => {
    ...
    showToast(`Removed ${e ? fmtDec(e.amount) : ""} ${e?.category || ""} entry`);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {!showForm ? (
        <button
          ...
          style={{ background: "linear-gradient(135deg,#00e5a0,#00b37a)", border: "none", borderRadius: 12, padding: "14px", color: "#0a0b12", fontSize: 14, fontWeight: 800, cursor: "pointer", width: "100%", letterSpacing: 0.3 }}
        >
          + Add Expense
        </button>
      ) : (
      ...
      <div style={{ display: "flex", gap: 4, background: "#12131a", padding: 3, borderRadius: 8, border: "1px solid #1e1f2e" }}>
        {[["today", "Today"], ["week", "This Week"], ["all", "All Time"]].map(([k, l]) => (
          <button
            key={k}
            onClick={() => setFilter(k)}
            style={{ flex: 1, padding: "6px 0", border: "none", borderRadius: 6, fontSize: 10, fontWeight: 700, cursor: "pointer", background: filter === k ? "#00e5a0" : "transparent", color: filter === k ? "#0a0b12" : "#64748b", transition: "all 0.2s" }}
          >
            {l}
          </button>
        ))}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 10, color: "#64748b", marginTop: 1 }}>
                    {e.note && <span style={{ color: "#94a3b8" }}>{e.note} · </span>}
                    {e.date}
                  </div>
                </div>
              </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// NET WORTH TAB — helper components
// ─────────────────────────────────────────────────────────────────────────────
function NetWorthEntryForm({ type, form, setForm, formError, onSave, onCancel, categories }) {
  const inputStyle = { width: "100%", background: "#1e1f2e", border: "1px solid #2a2b3d", borderRadius: 8, color: "white", fontSize: 14, padding: "10px 12px", outline: "none", fontFamily: "inherit" };
  return (
    <Card style={{ border: `1px solid ${type === "asset" ? "#00e5a030" : "#ff4d6d30"}` }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
          <button onClick={onSave} style={{ flex: 1, background: type === "asset" ? "#00e5a0" : "#ff4d6d", border: "none", borderRadius: 8, padding: "11px", color: type === "asset" ? "#0a0b12" : "white", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>
            Save {type === "asset" ? "Asset" : "Liability"}
          </button>
          <button onClick={onCancel} style={{ background: "#1e1f2e", border: "1px solid #2a2b3d", borderRadius: 8, padding: "11px 16px", color: "#64748b", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Cancel</button>
        </div>
      </div>
    </Card>
  );
...
function NetWorthListItem({ entry, onDelete, color }) {
...
// NET WORTH TAB
// ─────────────────────────────────────────────────────────────────────────────
function NetWorthTab({ monthlyLeak }) {
  const [assets, setAssets] = useLocalStorage(STORAGE_KEYS.assets, []);
  const [liabilities, setLiabilities] = useLocalStorage(STORAGE_KEYS.liabilities, []);
  ...
  const handleAdd = (type) => {
    ...
    setFormError("");
    const entry = { id: genId("n"), name: form.name.trim(), amount: amt, category: form.category };
    if (type === "asset") {
  ...
  const onCancelForm = () => { setShowForm(null); setFormError(""); setForm({ name: "", amount: "", category: "" }); };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${netWorth >= 0 ? "#0d2b1f" : "#2b0d0d"}, #0a0b12)`, border: `1px solid ${netWorth >= 0 ? "#00e5a030" : "#ff4d6d30"}`, borderRadius: 16, padding: 18 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
          <div>
            ...
            <p style={{ fontFamily: "monospace", fontSize: 28, fontWeight: 800, color: netWorth >= 0 ? "#00e5a0" : "#ff4d6d", lineHeight: 1 }}>{fmtDec(netWorth)}</p>
          </div>
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
            ...
            {totalLiabs > 0 && (
              <span style={{ fontSize: 10, fontFamily: "monospace", color: status.color, fontWeight: 700 }}>{coveragePct === Infinity ? "∞" : fmtPct(coveragePct)} coverage</span>
            )}
          </div>
      ...
      {!showForm && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <button onClick={() => { setShowForm("asset"); setForm({ name: "", amount: "", category: "" }); setFormError(""); }} style={{ background: "#00e5a015", border: "1px solid #00e5a030", borderRadius: 10, padding: "12px", color: "#00e5a0", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>+ Add Asset</button>
          <button onClick={() => { setShowForm("liability"); setForm({ name: "", amount: "", category: "" }); setFormError(""); }} style={{ background: "#ff4d6d15", border: "1px solid #ff4d6d30", borderRadius: 10, padding: "12px", color: "#ff4d6d", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>+ Add Liability</button>
        </div>
      )}
      {showForm && (
        <NetWorthEntryForm
          ...
          setForm={setForm}
          formError={formError}
          onSave={() => handleAdd(showForm)}
          onCancel={onCancelForm}
          categories={showForm === "asset" ? ASSET_CATS : LIAB_CATS}
        />
      )}
      ...
      <Card style={{ border: "1px solid #00e5a020" }}>
        ...
        {assets.length === 0 ? (
          ...
          assets.map((a) => <NetWorthListItem key={a.id} entry={a} onDelete={deleteAsset} color="#00e5a0" />)
        )}
      </Card>
      ...
      <Card style={{ border: "1px solid #ff4d6d20" }}>
        ...
        {liabilities.length === 0 ? (
          ...
          liabilities.map((l) => <NetWorthListItem key={l.id} entry={l} onDelete={deleteLiab} color="#ff4d6d" />)
        )}
      </Card>
...
export default function DTITracker() {
  ...
  const [credit, setCredit] = useLocalStorage(STORAGE_KEYS.credit, DEFAULT_CREDIT);
  ...
  const monthlyLeak = flaggedMonthly;

  const dti = totalIncomeMonthly > 0 ? ((totalExpensesMonthly + totalMinPay) / totalIncomeMonthly) * 100 : 0;
  const leftover = totalIncomeMonthly - totalExpensesMonthly - totalMinPay;
  const zone = getZone(dti);

  const [assets] = useLocalStorage(STORAGE_KEYS.assets, []);
  const [liabilities] = useLocalStorage(STORAGE_KEYS.liabilities, []);
  const netWorthXTab = useMemo(
    () => assets.reduce((s, a) => s + a.amount, 0) - liabilities.reduce((s, l) => s + l.amount, 0),
    [assets, liabilities]
  );
  ...
  const pieData = useMemo(() => {
    ...
    expenses.forEach((e) => {
      ...
      // Normalize recurring to monthly for a fair pie
      const monthly =
        e.frequency === "one-time"
          ? 0
          : (Number(e.amount) || 0) *
            ({ daily: 30.4375, weekly: 4.34524, biweekly: 2.17262, monthly: 1, quarterly: 1 / 3, yearly: 1 / 12 }[e.frequency || "monthly"] || 1);
      const oneTime =
        e.frequency === "one-time" && (e.date || "").slice(0, 7) === new Date().toISOString().slice(0, 7)
          ? Number(e.amount) || 0
          : 0;
      const val = monthly + oneTime;
      if (val > 0) g[e.category] = (g[e.category] || 0) + val;
    });
  ...
  const barData = useMemo(
    () => [
      { name: "Mo", income: totalIncomeMonthly, expenses: totalExpensesMonthly },
      { name: "Q", income: totalIncomeMonthly * 3, expenses: totalExpensesMonthly * 3 },
      { name: "Year", income: totalIncomeMonthly * 12, expenses: totalExpensesMonthly * 12 },
    ],
    [totalIncomeMonthly, totalExpensesMonthly]
  );

  const updateCredit = (id, field, val) => setCredit((prev) => prev.map((r) => (r.id === id ? { ...r, [field]: val } : r)));
  const addCredit = () =>
    setCredit((prev) => [...prev, { id: `c${Date.now()}`, label: "New Account", balance: 0, limit: 0, minPayment: 0, apr: 0, category: "Credit Card" }]);
  const savingsOpps = expenses.filter((e) => e.flag && SAVINGS_TIPS[e.id]);

  const topTabs = ["overview", "income", "expenses", "receipts", "credit", "savings"];
  ...
  return (
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif", background: "#0a0b12", minHeight: "100vh", color: "white", padding: "20px 14px 48px", maxWidth: 480, margin: "0 auto" }}>
            ...
            <button
              onClick={() => {
                if (window.confirm("Reset ALL data to defaults?")) {
                  ...
                  localStorage.removeItem("ml_category_overrides");
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
        {[
          { label: "Income/mo", value: fmt(totalIncomeMonthly), color: "#00e5a0" },
          { label: "Expenses/mo", value: fmt(totalExpensesMonthly), color: "#f87171" },
      ...
      <div style={{ display: "flex", gap: 3, marginBottom: 4, background: "#12131a", padding: 4, borderRadius: 10, border: "1px solid #1e1f2e", overflowX: "auto" }}>
        {topTabs.map((t) => (
      ...
      <div style={{ display: "flex", gap: 3, marginBottom: 14, background: "#12131a", padding: 4, borderRadius: 10, border: "1px solid #1e1f2e" }}>
        {bottomTabs.map((t) => (
          <button key={t} style={{ ...ts(t, "#60a5fa"), flex: 1, fontSize: 11 }} onClick={() => setTab(t)}>
            {allLabels[t]}
          </button>
        ))}
      </div>
      ...
      {tab === "overview" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Card>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              {[
                { label: "Monthly", inc: totalIncomeMonthly, exp: totalExpensesMonthly },
                { label: "Quarterly", inc: totalIncomeMonthly * 3, exp: totalExpensesMonthly * 3 },
                { label: "Yearly", inc: totalIncomeMonthly * 12, exp: totalExpensesMonthly * 12 },
              ].map((p) => (
          ...
          {/* Monthly Summary — NEW */}
          <MonthlySummary expenses={expenses} income={income} compact />
          ...
          <Card>
            ...
            {pieData.length === 0 ? (
              <p style={{ fontSize: 12, color: "#64748b", textAlign: "center", padding: "20px 0" }}>
                Add expenses to see your category mix.
              </p>
            ) : (
              <>
                  ...
                  {pieData.map((d) => (
                    <div key={d.name} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                      ...
                      <span style={{ fontSize: 10, color: "#94a3b8" }}>
                        {d.name} <span style={{ color: "#e2e8f0", fontFamily: "monospace" }}>{fmt(d.value)}</span>
                      </span>
                    </div>
                  ))}
                </div>
          ...
          <FinancialRealityCheck totalIncome={totalIncomeMonthly} monthlyLeak={monthlyLeak} netWorth={netWorthXTab} />
      ...
      {tab === "income" && (
        <IncomeList
          ...
          period={incomePeriod}
          onChangePeriod={setIncomePeriod}
        />
      )}
      ...
      {tab === "expenses" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <ExpenseList
            ...
            onToggleFlag={toggleExpenseFlag}
            period={expensePeriod}
            onChangePeriod={setExpensePeriod}
          />
        </div>
      )}
            ...
            return (
              <Card key={acct.id}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 16px" }}>
                  {[
                    { label: "Balance", field: "balance", prefix: "$" },
                    { label: "Credit Limit", field: "limit", prefix: "$" },
                    { label: "Min Payment", field: "minPayment", prefix: "$" },
                    { label: "APR", field: "apr", suffix: "%" },
                  ].map((f) => (
                    <div key={f.field}>
                ...
                {acct.limit > 0 && (
                  <>
                    ...
                    <p style={{ fontSize: 9, color: "#64748b", marginTop: 4 }}>
                      {util >= 70 ? "⚠️ High utilization. Hurting your credit score." : util >= 30 ? "Moderate. Aim for under 30%." : "✅ Good. Under 30%."}
                    </p>
                  </>
                )}
              </Card>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ background: "#1a0f00", border: "1px solid #ff8c4228", borderRadius: 12, padding: 14 }}>
            ...
            <p style={{ fontFamily: "monospace", fontSize: 24, fontWeight: 800, color: "#ff4d6d" }}>
              {fmt(flaggedMonthly)}
              <span style={{ fontSize: 12, color: "#64748b", marginLeft: 6, fontFamily: "sans-serif" }}>/mo</span>
            </p>
            <div style={{ display: "flex", gap: 20, marginTop: 8 }}>
              <div>
                <div style={{ fontSize: 9, color: "#64748b", textTransform: "uppercase" }}>Quarterly</div>
                <div style={{ fontSize: 13, fontFamily: "monospace", color: "#ff8c42", fontWeight: 700 }}>{fmt(flaggedMonthly * 3)}</div>
              </div>
              <div>
                <div style={{ fontSize: 9, color: "#64748b", textTransform: "uppercase" }}>Yearly</div>
                <div style={{ fontSize: 13, fontFamily: "monospace", color: "#ff8c42", fontWeight: 700 }}>{fmt(flaggedMonthly * 12)}</div>
              </div>
            </div>
          </div>

          <MonthlySummary expenses={expenses} income={income} />
            ...
            <div style={{ display: "flex", gap: 14 }}>
              {[["Monthly", 1], ["Quarterly", 3], ["Yearly", 12]].map(([l, m]) => (
                <div key={l}>
                  ...
                  <div style={{ fontSize: 14, fontFamily: "monospace", color: "#60a5fa", fontWeight: 800 }}>+{fmt(flaggedMonthly * 0.5 * m)}</div>
                </div>
              ))}
            </div>
      ...
      {/* NET WORTH */}
      {tab === "networth" && <NetWorthTab monthlyLeak={monthlyLeak} />}
    </div>
  );
}
